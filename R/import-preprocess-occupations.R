...
# To do: add "losateo"?
#
wellbeing <- gather_hilda(hilda, c("ghmh", "losat", "pdk10s")) %>%
  spread(code, val) %>% 
  mutate_if(is.double, ~ ifelse(. < 0, NA_real_, .))
...
job_security <- c(
  ...
  "jomwf", # I worry about the future of my job [all waves] *reverse
...
# Waves 5:20
demand6 <- combine_scores(hilda, job_demand6) %>% 
  rename(job_demand6 = combined)

complexity <- combine_scores(hilda, job_complexity) %>%
  rename(job_complexity = combined)

resources <- combine_scores(hilda, job_resources) %>%
  rename(job_resources = combined)

# Waves 1:20 (create job demand score from 9 items)
...
preprocessed %>%
  ...
  write_rds("data/teachers & nurses.rds")
