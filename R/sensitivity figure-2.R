...
dmdctl_f <- formula(ghmh ~ s(year) + cage + cage:year + sex + sex:year +
                     jbempt + jbempt:year +
                     s(year, by = rjob_control) + rjob_control +
                     # s(year, by = ajob_resources) + ajob_resources + 
                     s(year, by = ajob_demand) + ajob_demand)
...
fit_t_dmdctl <- fit_ghmh(model.df, .prof = "Teachers", .f = dmdctl_f)
...
fit_n_dmdctl <- fit_ghmh(model.df, .prof = "Nurses", .f = dmdctl_f)
...
df_t <- bind_rows(
  ...
  `Model V` = smooth_data(fit_t_dmdctl),
  .id = "model"
) %>%
...
df_n <- bind_rows(
  ...
  `Model V` = smooth_data(fit_n_dmdctl),
  .id = "model"
) %>%
