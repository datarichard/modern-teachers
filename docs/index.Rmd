...
bind_rows(
  `Mental health` = select(df, year, ghmh.betas) %>%
    unnest(ghmh.betas),
  `Life satisfaction` = select(df, year, losat.betas) %>%
    unnest(losat.betas),
  .id = "model"
) %>%
  bind_rows(
    fits %>%
      transmute(
        term = "r-squared",
        `Mental health` = map_dbl(ghmh.fit, ~glance(.) %>% pull(r.squared)),
        `Life satisfaction` = map_dbl(losat.fit, ~glance(.) %>% pull(r.squared))
      ) %>%
      gather("model", "estimate", `Mental health`:`Life satisfaction`)
  ) %>%
  filter(term %notin% "(Intercept)") %>%
  mutate(model = fct_relevel(model, "Mental health"),
         term = if_else(str_detect(term, "job"), paste(term, "(\u03B2)"), term)) %>%
  # filter(year > 2010) %>%
  ggplot(aes(x = year, y = estimate, color = term)) +
    ...
    labs(title = "Effect of job characteristics on mental health 
and life satisfaction in each year\n", 
         x = "", y = "") +
    theme_economist() +
    scale_color_manual(values = c("#c72e29", "#016392", "#be9c2e", "grey50")) +
    # scale_color_wsj() +
    theme(legend.position = "none",
...
Mental health seems to be increasingly dependent on job control and job demands in the last ten years, as the beta estimates and model r-squared values get further from zero between 2011 to 2020. The dependence of life satisfaction on job characteristics has remained  relatively steady over the same time period.  
...
teachers %>%
  ...
  mutate(
    fit = map(data, ~lm(
      formula = ghmh ~ 1 + remoteness + job_control:remoteness + job_demand:remoteness + 
